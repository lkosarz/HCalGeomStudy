...
void plotAll() {
  ...
  for (auto &input : inputFiles) {
    ...
    if (!energyDeposition.back()) std::cerr << "energyDeposition is null" << std::endl << std::endl;
  }
  ...
  hists.push_back(energyDeposition);

  std::vector<TString> histsTitles = {"#phi resolution", "#theta resolution", "energy deposition"};
  std::vector<TString> Xlabels = {"#Delta#phi [#circ]", "#Delta#theta [#circ]", "Energy [GeV]"};
...
///////////////////////////////////// ENERGY DEPOSITION ///////////////////////////////////////
  ...
  for (unsigned int j = 0; j < hists[2].size(); j++) {
    can->cd(j+1);
    
    gPad->SetRightMargin(0.02);
    if (j==2 || j==3) gPad->SetTopMargin(0.05);
    gPad->SetBottomMargin(0.15);
    gPad->SetLeftMargin(0.13);
    
    hists[2][j]->Draw();
    if (j==0) hists[2][j]->Rebin(2);
    if (j==1) hists[2][j]->Rebin(2);
    if (j==2) hists[2][j]->Rebin(2);
    if (j==3) hists[2][j]->Rebin(2);

    if (j==0) hists[2][j]->SetAxisRange(0, 3,"X");

    hists[2][j]->SetTitle("");
    hists[2][j]->GetXaxis()->SetTitle(Xlabels[2]);
    hists[2][j]->GetYaxis()->SetTitle("Counts");
    hists[2][j]->GetXaxis()->SetTitleOffset(0.9);
    hists[2][j]->GetYaxis()->SetTitleOffset(0.9);

    if (j==0) hists[2][j]->SetLineColor(kRed+2);
    if (j==1) hists[2][j]->SetLineColor(kBlack);
    if (j==2) hists[2][j]->SetLineColor(kGreen+2);
    if (j==3) hists[2][j]->SetLineColor(kBlue+1);

    hists[2][j]->SetLineWidth(2);
    hists[2][j]->SetStats(0);

    hists[2][j]->GetXaxis()->SetTitleSize(0.07);
    hists[2][j]->GetYaxis()->SetTitleSize(0.07);
    hists[2][j]->GetXaxis()->SetLabelSize(0.07);
    hists[2][j]->GetYaxis()->SetLabelSize(0.07);

    if (j==0 || j==1) text->DrawLatex(0.65, 0.72, Form("#splitline{%s}{%s GeV}", histsTitles[2].Data(), energyTitles[j].Data()));
    if (j==2 || j==3) text->DrawLatex(0.65, 0.77, Form("#splitline{%s}{%s GeV}", histsTitles[2].Data(), energyTitles[j].Data()));
