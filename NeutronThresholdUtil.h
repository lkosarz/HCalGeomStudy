...
namespace NeutronThresholds {
    ...
    const std::vector<double> T_MAX = {25, 100, 500, 1000, 10000};
    ...
    void processContributions(const ContribCollection& contrib, std::vector<std::vector<double>>& hits_passed, std::vector<std::vector<double>>& hits_passed_telap, TH1* h_contrib_time = nullptr, TH1* h_contrib_energy = nullptr, bool debug = false
    ) {
        ...
        std::cout << contrib.at(0).getTime() << std::endl;
        ...
        for (int iet = 0; iet < E_TH.size(); iet++) {
            for (int ies = 0; ies < E_sum.size(); ies++) {
                if (E_sum[ies] > E_TH[iet]) {
                    hits_passed[iet][ies] += 1;
                }
                if (E_sum_telap[ies] > E_TH[iet]) {
        ...
        for (int iet = 0; iet < E_TH.size(); iet++) {
            for (int itm = 0; itm < T_MAX.size(); itm++) {
                if (hits_passed[iet][itm] > 0) {
                    h_energy_vs_time->Fill(E_TH[iet], T_MAX[itm]);
                }
                if (hits_passed_telap[iet][itm] > 0) {
                    h_energy_vs_telap->Fill(E_TH[iet], T_MAX[itm]);
                }
                h_energy_vs_time_total->Fill(E_TH[iet], T_MAX[itm]);
            }
        }
    }
}
